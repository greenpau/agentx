...
`SKILL-004` — Bare or untrusted-workspace mode loads no skills. User configuration, managed roots, nested roots, explicit external directories, plugins, bundles, legacy commands, and remote providers never contribute AgentX skills.
...
`SKILL-011` — Resolve only `<repository-root>/.codex/skills`, canonicalize it, and scan its direct skill directories deterministically. No source-precedence merge exists because there is exactly one eligible root.
...
`SKILL-022` — A malformed individual skill is omitted with its path and schema diagnostic. Other skills in the root remain available. Treat invalid boolean strings, unsupported context values, and malformed hooks as validation failures for the affected capability, not truthy values.
...
`SKILL-031` — Listing character budget is approximately one percent of the selected model context, using four characters per token; if model context is unavailable, use an 8,000-character fallback. Cap each nonbundled description at 250 characters before global allocation.

`SKILL-032` — Preserve full bundled-skill descriptions where possible. When remaining budget would allocate fewer than 20 description characters per nonbundled skill, list nonbundled names without descriptions rather than arbitrary unusable fragments.
...
## Bundled extraction security

`SKILL-070` — Extract bundled skills under the protected runtime temp root using a product/version component and random 16-byte nonce. Directory access is owner-only.
...
| root unreadable | report provider/root failure; preserve other roots |
...
2. **SKILL-A02 — Canonical-root deduplication.** User and project roots resolve through symlinks to the same directory. The first canonical root loads once; no duplicate appears.
