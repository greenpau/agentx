...
| Credential/configuration normalization | `pkg/config` | strict version/provider/schema, duplicate/unknown/trailing/oversize rejection, sole-source enforcement, Unix mode-bit enforcement, Windows identity/link checks, redaction, endpoint, model/effort tests |
...
| Authentication and provider selection | Partial | The sole model credential source is one strict version-1, provider-discriminated application-home `auth.json` Azure subscription key. The literal child is opened through the frozen home descriptor, not by re-resolving a mutable pathname. POSIX files require effective-user ownership and private mode bits. Windows model-backed authentication is unavailable: regular-file, non-symlink, single-link, stable-identity, and bounded-read code exists, but every load fails before reading because native owner/DACL verification is absent. OAuth, Bedrock, Vertex, first-party account login, and provider/model fallback are unavailable |
