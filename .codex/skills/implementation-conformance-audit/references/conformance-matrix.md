...
| `CONF-010` | Headless/SDK | initialize, prompt, control request, result, idle; provider-free native session list and one revision-bound deletion | malformed NDJSON, late duplicate response, EOF, rejected conversation-option mix, stale page/revision, lock contention, and cleanup-pending status | single FIFO output and correlated requests for SDK mode; management emits only its one versioned object and never starts a semantic session |
...
| `CONF-016` | Transcript/recovery | append, flush, resume, fork, workspace inventory, and identity-bound delete | truncated tail, orphan tool pair, DAG sibling, incomplete fork, unsafe/link/replacement identity, concurrent selection/delete, and crash at each deletion commit boundary | recovery preserves evidence and does not invent success; inventory and all selectors share one authority, and retained session data is never called deleted |
