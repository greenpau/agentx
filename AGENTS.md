...
- Admit caller-selected files only through the versioned attachment boundary. Snapshot and validate supported media into session-owned immutable storage, persist bounded manifests rather than bytes or source paths, preserve typed content order through queueing and recovery, and resolve bytes only at a qualified provider boundary.
...
5. Accept user input or an equivalent external event; import and validate supported attachments before admission; expand supported references; distinguish local commands from model-bound prompts; run input hooks; queue, reject, redact, or persist the complete typed input as its contract requires.
...
| `pkg/attachment/` | Import caller-selected media, validate and normalize its bytes, and own immutable session attachment manifests, blobs, uploads, reservations, and collection. |
